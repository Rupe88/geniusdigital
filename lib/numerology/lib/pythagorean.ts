import { getPythagoreanValue } from '../constants/pythagorean';
...
export function calculatePythagorean(input: string): PythagoreanResult | null {
  ...
  for (const char of trimmed) {
    if (/[A-Za-z]/.test(char)) {
      const v = getPythagoreanValue(char.toUpperCase());
