...
export default function LiveClassDetailPage() {
  ...
  const handleEnroll = async () => {
    if (!id || !isAuthenticated) {
      router.push(`/login?redirect=${encodeURIComponent(`/live-classes/${id}`)}`);
      return;
    }
    setEnrolling(true);
    try {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[var(--muted)]/30 py-12">
        <div className="max-w-3xl mx-auto px-4 flex flex-col items-center justify-center py-20">
        ...
        <div className="max-w-3xl mx-auto px-4">
          <Card padding="lg" className="text-center">
            ...
            <div className="flex gap-3 justify-center">
              <Button variant="outline" onClick={() => router.push(ROUTES.LIVE_CLASSES)}>
          ...
          <div className="flex flex-wrap gap-3 pt-4 border-t border-[var(--border)]">
            ...
            {!isAuthenticated && liveClass.status !== 'CANCELLED' && liveClass.status !== 'COMPLETED' && (
              <Button
                variant="primary"
                size="lg"
                onClick={() => router.push(`/login?redirect=${encodeURIComponent(`/live-classes/${id}`)}`)}
                className="gap-2"
              >
                Log in to enroll
              </Button>
